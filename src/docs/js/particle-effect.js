...
class ParticleAnimation {
  constructor(el, { quantity = 30, staticity = 90, ease = 70, maxSize = 2 } = {}) {
    this.canvas = el;
  ...
  init() {
    ...
    // this fires when mobile navbars appear or disappear. This is undesired behavior.
  ...
  circleParams() {
    ...
    const translateX = 0;
    const translateY = 0;
    // Size can be between 1 and maxSize (default is 2)
    const size = Math.floor(Math.random() * this.settings.maxSize) + 1;
    const alpha = 0;
  ...
  drawCircle(circle, update = false) {
    ...
    const accentColor = computedStyle.getPropertyValue("--accent").trim() || "rgb(255, 255, 255)";

    this.context.translate(translateX, translateY);
    this.context.beginPath();
    this.context.arc(x, y, size, 0, 2 * Math.PI);

    // Set global alpha instead of using rgba
...
const startParticles = () => {
  ...
  canvasElements.forEach((canvas) => {
    const options = {
      quantity: parseInt(canvas.dataset.particleQuantity, 10) || 30,
      staticity: parseInt(canvas.dataset.particleStaticity, 10) || 90,
      ease: parseInt(canvas.dataset.particleEase, 10) || 70,
      maxSize: parseInt(canvas.dataset.particleMaxSize, 10) || 2,
    };
